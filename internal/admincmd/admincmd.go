...
// 容器只需要 nexacard-api 一个二进制即可执行所有运维操作。
package admincmd
...
用法:
  nexacard-api admin list-admins                          列出所有管理员
  nexacard-api admin reset-2fa --username <name>          重置指定管理员的 2FA
  nexacard-api admin reset-password --username <name> [--password <new>]
                                                          重置管理员密码（超管忘记密码恢复用）
